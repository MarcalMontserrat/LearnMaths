...
export function GuidedNotebook({ question, isSolved, inputRef, onAnswerChange }) {
  ...
  const updateResultNote = (index, nextValue) => {
    setResultNotes((currentNotes) => {
      const nextNotes = currentNotes.map((note, noteIndex) =>
        noteIndex === index ? sanitizeNotebookValue(nextValue, 1) : note
      );

      onAnswerChange(buildScratchAnswer(nextNotes));
      ...
      <div className="notebook-header">
        <div className="notebook-copy">
          <span className="section-kicker">
            {layout.variant === 'long-multiplication' ? 'Pizarra larga' : 'Pizarra en columnas'}
          </span>
          <h3>{layout.title}</h3>
          ...
          <>
            ...
            <div className="notebook-row">
              ...
              {layout.resultSlots.map((slot, index) => (
                <input
                  key={slot.id}
                  ref={index === layout.resultSlots.length - 1 ? inputRef : null}
                  className="notebook-input notebook-result-input"
                  inputMode="numeric"
                  pattern="[0-9]*"
                  autoComplete="off"
